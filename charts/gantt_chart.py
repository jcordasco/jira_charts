...
def gantt_chart(df, export_path=None):
    logger.info("Generating Gantt chart v2.8 (non-blocking rendering)...")
        ...
        for _, row in team_rows.iterrows():
            ...
            for level in range(len(levels) + 1):
                ...
                for other_start, other_end in levels[level] if level < len(levels) else []:
                    if start <= (other_end + timedelta(days=1)):
                        conflict = True
                        break
    ...
    ax.set_title("Jira Gantt Chart (Visual Swimlanes, Dividers, Today Marker, Day Gridlines)")
    plt.tight_layout()
